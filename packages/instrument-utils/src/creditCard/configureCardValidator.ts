...
interface MutableCreditCardType {
    getTypeInfo(type: string): {
        lengths?: number[];
        patterns?: Array<number | [number, number]>;
    };
    updateCard(type: string, updates: Record<string, unknown>): void;
    addCard(config: Record<string, unknown>): void;
}

export default function configureCardValidator(): void {
    const cardType = creditCardType as unknown as MutableCreditCardType;
    const discoverInfo = cardType.getTypeInfo('discover');
    const visaInfo = cardType.getTypeInfo('visa');

    // Need to support 13 digit PAN because some gateways only provide test credit card numbers in this format.
    cardType.updateCard('visa', {
        lengths: [13, ...(visaInfo.lengths || [])],
    });

    // Add support for 8-BIN Discover Cards.
    cardType.updateCard('discover', {
