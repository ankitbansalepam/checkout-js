...
import { ShopPayOrderConfirmation } from '../order/ShopPayOrderConfirmation';
...
const Checkout = ({
    ...
    const [shopPayConfirmation, setShopPayConfirmation] = useState<
        { orderId: number; confirmationToken: string } | undefined
    >(() => {
        const params = new URLSearchParams(window.location.search);
        const orderId = Number(params.get('orderId'));

        return params.get('shopPay') === '1' && orderId
            ? { orderId, confirmationToken: params.get('confirmationToken') || '' }
            : undefined;
    });
    ...
    useEffect(() => {
        const handlePopState = () => {
            ...
            setShopPayConfirmation(
                params.get('shopPay') === '1' && orderId
                    ? { orderId, confirmationToken: params.get('confirmationToken') || '' }
                    : undefined,
            );
        };
    ...
    if (shopPayConfirmation) {
        return (
            <ShopPayOrderConfirmation
                confirmationToken={shopPayConfirmation.confirmationToken}
                orderId={shopPayConfirmation.orderId}
            />
        );
    }
